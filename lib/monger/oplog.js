...
var Timestamp = require('bson').Timestamp,
  ...
  debug = require('debug')('monger:oplog');
...
Oplog.prototype.since = function(ts){
  ...
  debug('selector updated', this.selector);
  return this;
};
...
Oplog.prototype.find = function(which, opts){
  ...
  debug('created cursor', this.cursor, this.selector, opts);

  if(opts.tailable === 1){
    this.cursor = this.cursor.each(this.each.bind(this));
    debug('bound tailable processor');
  }
  return this;
};
