  ...
  async.parallel([
    ...
    self.db.collection('mongos').find({}).toArray(function(err, docs){
      ...
      docs.map(function(doc){
        var instance = types.url(doc._id).toJSON();
          instance.type = 'router';
...
Sharding.prototype.getCollectionDetails = function(coll, fn){
    ...
    self.db.collection('chunks').find({"ns" : coll._id}).sort( { min : 1 } ).toArray(function(err, data){
      ...
      data.map(function(doc){
        var chunk = {
          _id: doc._id,
          lastmod: doc.lastmod,
          lastmodEpoch: doc.lastmodEpoch,
          keyspace: [doc.min, doc.max]
        };
        // res.shards[chunk.shard].chunks.push(chunk);
      });
      done();
    });
  }
  ...
  async.series([getStats, getTags], function(err){
    ...
    async.parallel(Object.keys(shards).map(function(shard_id){
      return function(cb){
        getShardDetail(shard_id, cb);
      };
    }), function(err){
      if(err) return fn(err);
