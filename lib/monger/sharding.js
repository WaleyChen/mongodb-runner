...
Sharding.prototype.balancer = function(fn){
  var state = true, running = false;
  this.db.collection('settings').findOne({_id: 'balancer'}, function(err, doc){
    if(err) return fn(err);
    if(doc){
      state = !doc.stopped;
    }
    this.db.collection('locks').findOne({_id: 'balancer'}, function(err, doc){
      if(err) return fn(err);
      if(doc){
        running = doc.state > 0;
      }
...
Sharding.prototype.getConfigServers = function(fn){
  this.db.db('admin').command({getCmdLineOpts: 1}, function(err, data){
    if(err) return fn(err);
...
Sharding.prototype.getCollectionStats = function(collectionId, done){
  ...
  this.db.db(ns.database).command({collStats: ns.collection}, function(err, data){
    if(err) return done(err);
...
Sharding.prototype.getCollectionTags = function(collectionId, done){
  this.db.collection('tags').find({ns: collectionId}).sort({min: 1}).toArray(done);
};
...
Sharding.prototype.getShards = function(fn){
  this.db.collection('shards').find({}).toArray(function(err, docs){
    ...
    var tasks = docs.map(function(doc){
      return function(cb){this.getShardDetail(doc._id, cb);}.bind(this);
    }.bind(this));
...
Sharding.prototype.getChunks = function(done){
  this.db.collection('chunks').find({}).sort( { min : 1 } ).toArray(function(err, data){
    if(err) return done(err);
...
Sharding.prototype.getShardDetail = function(shardId, done){
  ...
  this.db.collection('shards').find({_id: shardId}).toArray(function(err, data){
    if(err) return done(err);
