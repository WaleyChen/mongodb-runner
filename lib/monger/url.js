...
function parse(url, options) {
  ...
  if(url.indexOf('mongodb://') !== 0){
    throw new Error('URL must be in the format mongodb://user:pass@host:port/dbname');
  }
  ...
  urlOptions.forEach(function(opt) {
    ...
    switch(name) {
      ...
      case 'authMechanism':
        ...
        if(value !== 'GSSAPI' && value !== 'MONGODB-X509' &&
            value !== 'MONGODB-CR' && value !== 'PLAIN'){
  ...
  if((dbOptions.w === -1 || dbOptions.w === 0) && (
      dbOptions.journal === true || dbOptions.fsync === true ||
      dbOptions.safe === true)){
...
Object.defineProperty(URL.prototype, 'id', {get: function(){
  var host = this.data.servers[0].host.toLowerCase(),
    id = 0;
  if(host !== 'localhost'){
    host.split('').map(function(c, i){id += host.charCodeAt(i);});
  }
  if(this.data.auth){
    var username = this.data.auth.user;
    username.split('').map(function(c, i){id += username.charCodeAt(i);});
  }
  id += this.data.servers[0].port;
  return id.toString(24);
}});

Object.defineProperty(URL.prototype, 'name', {get: function(){
  return this.data.servers[0].host + ':' + this.data.servers[0].port;
}});

Object.defineProperty(URL.prototype, 'auth', {get: function(){
  return this.data.auth;
}});

Object.defineProperty(URL.prototype, 'dbName', {get: function(){
...
URL.prototype.toJSON = function(){
  return {_id: this.id, name: this.name, url: this.toString(), auth: this.auth};
};
