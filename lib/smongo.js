var stream = require('stream'),
  ...
  debug = require('debug')('mongoscope:smongo');

function middleware(klass, app){
  return function(req, res){
    var stream = Poller.getInstance(klass, req.mongo.admin(), app);
...
Poller.getInstance = function(klass, db, app){
  var name = klass.prototype.name;

  if(!Poller.instances[name]) Poller.instances[name] = {};

  if(!Poller.instances[name][db.id]){
    Poller.instances[name][db.id] = new klass(db);
    // @todo: add token challenge before opening socketio back up.
    var io = app.get('io'),
      uri = '/' + klass.prototype.name;

    io.sockets.on('connection', function(socket){
      socket.on(uri, function(host){
        deployment.get(host, function(err, dep){});
