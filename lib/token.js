...
module.exports.required = function(req, res, next){
  ...
  if(req.param('host')) req.instance_name = req.param('host');
...
function getContext(token, ctx, next){
    ...
    Deployment.get(data.deployment_id, function(err, deployment){
      ...
      if(ctx.instance_name){
        debug('getting connection to instance', ctx.instance_name);

        ctx.instance = deployment.getInstance({name: ctx.instance_name});

        if(!ctx.instance){
          return next(new Forbidden('Tried getting a connection ' +
            'to `' + ctx.instance_name + '` but it is not in deployment `'+data.deployment_id+'`'));
        }
