...
var authHelp = ['','',
  '----------------------------------------------------------------------',
  'You need to include the Authization header in your request like so:',
  '`Authorization: Bearer {{token}}`',
  '',
  'A handy shortcut if you are in the shell:',
  'export TOKEN=`curl -H "Accept: text/plain" -s http://localhost:29017/api/v1/token -d "seed=localhost:27017"`;',
  '',
  'Use that token like so:',
  'curl -X GET -H "Authorization: Bearer $TOKEN" http://localhost:29017/api/v1',
  '',
  'For details, please see: http://scope.mongodb.land/docs#authentication',
  ''
].join('\n');
...
module.exports.required = function(req, res, next){
  ...
  if(!auth) return next(new NotAuthorized('Missing authorization header' + authHelp));

  access_token = parts[1];
  if(parts.length !== 2 || parts[0] !== 'Bearer'){
    return next(new BadRequest('Authorization header malformed.' + authHelp));
  }
  if(req.param('instance_id')){
    req.instance_id = Deployment.getId(req.param('instance_id'));
  }
