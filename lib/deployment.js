  ...
  connect(seed, function(err, db){
    ...
    db.command({ismaster: true}, function(err, res){
      ...
      getSeedType(res, function(err, type){
        ...
        if(type !== 'standalone'){
          if(!res.primary){
            debug('this is a router');
          //   mongos> db.mongos.find()
          // { "_id" : "Lucass-MacBook-Pro.local:30999", "ping" : ISODate("2014-04-18T22:38:57.502Z"), "up" : 5476, "waiting" : true, "mongoVersion" : "2.7.0-pre-" }
...
var getId = module.exports.id = function(uri){
...
function Instance(uri, type){
  if(!uri) throw new Error('Tried creating an instance with no uri: ' + uri + ' ' + type );
