...
Instance.prototype.addConnection = function(conn){
...
Deployment.prototype.addConnection = function(conn){
  debug('add connection', this);
  var name = conn.serverConfig.socketOptions.host + ':' + conn.serverConfig.socketOptions.port,
    inst = this.getInstance({name: name});
  if(!inst) throw new Error('No instance named ' + name + ' in this deployment');
    ...
    autoname(seed, function(err, name){
      ...
      generateId(seed, function(err, id){
        ...
        discover(conn, function(err, res){
          ...
          var deployment = new Deployment(data).addConnection(conn);
