var NotAuthorized = require('../errors').NotAuthorized,
  ...
  debug = require('debug')('mongoscope:instance');
...
module.exports = function(app){
  ...
  app.monger('/ops', monger.currentop);

  app.monger('/log', monger.log);
  app.monger('/top', monger.top);

  // @todo: should just be able to call /api/v1/sharding?
...
// a router returns a subset of the metrics
//
// ```
// { host: 'Lucass-MacBook-Pro.local:30999',
//   version: '2.7.0-pre-',
//   process: 'mongos',
//   pid: 1826,
//   uptime: 159823,
//   uptimeMillis: 159823577,
//   uptimeEstimate: 52002,
//   localTime: Wed Apr 30 2014 16:39:27 GMT-0400 (EDT),
//   asserts: { regular: 0, warning: 0, msg: 0, user: 45, rollovers: 0 },
//   connections: { current: 11, available: 193, totalCreated: 355 },
//   extra_info: { note: 'fields vary by platform', page_faults: 56 },
//   network: { bytesIn: 51821, bytesOut: 125152, numRequests: 634 },
//   opcounters:
//    { insert: 0,
//      query: 455,
//      update: 0,
...
//      getmore: 0,
//      command: 179 },
//   mem: { bits: 64, resident: 5, virtual: 2424, supported: true },
//   metrics: { getLastError: { wtime: [Object] } },
//   ok: 1 }
// ```
...
function commandError(data){
  debug('checking for command error', data);
  if(!data){
    return new BadRequest('No data from driver?');
  }
  if(data.ok === 0){
    return new BadRequest('Not ok?');
  }
  if(data.documents && data.documents[0].ok === 0){
    return new NotAuthorized(data.documents[0].errmsg);
  }
}
...
function info(req, res, next){
  ...
  req.mongo.admin().command({hostInfo: 1}, {}, function(err, data){
    if(err || (err = commandError(data))) return next(err);
    ...
    res.instance.host.disk_total = 0;
    res.instance.host.disk_free  = 0;
    res.instance.host.disk_state = 0;
    next();
...
function dbNames(req, res, next){
  req.mongo.admin().listDatabases(function(err, data){
    if(err || (err = commandError(data))) return next(err);
    debug('databases', data.databases);
