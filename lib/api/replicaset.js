...
module.exports = function(app){
  app.get('/api/v1/:host/replication', head, tail, size, details);
...
function head(req, res, next){
   oplog(req.mongo).since(0).limit(1).sort({$natural: 1}).first(function(err, doc){
    if(err) return next(err);
    req.rs_head = doc;
    next();
   });
...
function size(req, res, next){
  read(req.db, 'system.namespaces').where({name:'local.oplog.rs'}).first(function(err, doc){
    if(err) return next(err);
...
function details(req, res, next){
  req.mongo.admin().command({replSetGetStatus: 1}, {}, function(err, data){
    ...
    replicaset.oplog_window = [new Date(req.rs_head.ts), new Date(req.rs_tail.ts)];
