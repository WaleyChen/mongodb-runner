...
module.exports = function(app){
  ...
  //     [
  //       {
  //         "_id": "localinstance:27017",
  //         "instances": {
  //           "localinstance:27017": {
  //             "type": "standalone",
  //             "uri": "localinstance:27017"
  //           }
  //         }
  //       }
  //     ]
  ...
  function send(deployment, ctx, res, next){
    ...
    return token.create(ctx, function(err, data){
      if(err) return next(err);
      if(ctx.accept === 'text/plain'){
        return res.send(201, data.token);
      }
  ...
  app.post('/api/v1/token', bodyParser(), function(req, res, next){
    ...
    Deployment.get(Deployment.getId(url), function(err, deployment){
      if(err) return next(err);
  ...
  ['sharding', 'replication', 'instance', 'security', 'database', 'collection'].map(function(name){
    require('./' + name)(app);
  });
