...
module.exports = function(app){
  app.get('/api/v1/:instance_id/:database_name/:collection_name', stats, indexes, get);
  app.get('/api/v1/:instance_id/:database_name/:collection_name/count', read('count'));
  app.get('/api/v1/:instance_id/:database_name/:collection_name/find', read('find'));
  ...
  // @api public
  app.get('/api/v1/:instance_id/:database_name/:collection_name/aggregate', function(req, res, next){
  ...
  app.get('/api/v1/:instance_id/:database_name/:collection_name/distinct/:key',  distinct);
  app.get('/api/v1/:instance_id/:database_name/:collection_name/plans',  plans);

  app.post('/api/v1/:instance_id/:database_name/:collection_name/import', bodyParser(), importer);
};
...
function stats(req, res, next){
  req.db.command({collStats: req.param('collection_name'), verbose: 1}, {}, function(err, data){
    ...
    req.col.is_capped = data.capped;
    req.col.max = data.max;
    req.col.is_power_of_two = data.userFlags === 1;
    req.col.stats = {
