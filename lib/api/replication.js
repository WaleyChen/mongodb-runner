...
module.exports = function(app){
  app.get('/api/v1/:instance_id/replication', verify, head, tail, size, opcount, details);
  ...
  app.get('/api/v1/:instance_id/replication/watch', function(req, res){
    ...
    .on('left', function(err, member){
      es.emit('left', Deployment.getId(member.host + ':' + member.port));
    })
    .on('joined', function(err, member){
      es.emit('join', Deployment.getId(member.host + ':' + member.port));
    });
  });
