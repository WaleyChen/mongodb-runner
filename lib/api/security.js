...
module.exports = function(app){
  // @todo: This needs to be on the database level.
  app.get('/api/v1/:instance_id/security', users, roles, function(req, res){
    res.send({
  ...
  // @todo: This needs to be on the database level.
  app.get('/api/v1/:instance_id/security/users', users, function(req, res){
    res.send(req.mongo.users);
  });

  // @todo: This needs to be on the database level.
  app.get('/api/v1/:instance_id/security/roles', roles, function(req, res){
    res.send(req.mongo.roles);
  });

  app.get('/api/v1/:instance_id/security/users/:database_name/:username',  function(req, res, next){
  ...
  app.get('/api/v1/:instance_id/security/roles/:database_name/:role', function(req, res, next){
